...
export async function getCUACoordinates(instruction: string, screenshotUri: string): Promise<Coordinates> {
    const baseUrl = (process.env.AGENT_BASE_URL || process.env.BACKEND_BASE_URL || "https://playground.gbox.ai").replace(/\/$/, "");
