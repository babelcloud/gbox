...
export function restoreCoordinate(x: number, y: number, resizeRatio: number): { x: number; y: number } {
  // round to integer
  const restored = { x: Math.round(x / resizeRatio), y: Math.round(y / resizeRatio) };
  return restored;
}
 ...
 */
export async function resizeImage(buffer: Buffer, resizeRatio: number): Promise<Buffer> {
...
*/
export async function maybeResizeAndCompressImage(base64Data: string, mimeType: string, screenSize: { width: number; height: number }): Promise<{ base64Data: string; mimeType: string }> {
  try {
    const resizeRatio = calculateResizeRatio(screenSize.width, screenSize.height);
    const resizedBuffer = await resizeImage(Buffer.from(base64Data, 'base64'), resizeRatio);
    ...
    switch (mimeType) {
      case "image/jpeg":
      case "image/jpg": {
        outputBuffer = await sharp(resizedBuffer).jpeg({ quality: 80, mozjpeg: true }).toBuffer();
        break;
      }
      case "image/png": {
        outputBuffer = await sharp(resizedBuffer).png({ quality: 80, compressionLevel: 9, palette: true }).toBuffer();
        break;
      }
      case "image/gif": {
        outputBuffer = await sharp(resizedBuffer, { animated: true }).gif({ colours: 128 }).toBuffer();
        break;
      }
      default:
...
export async function getImageDataFromUri(uri: string, box: AndroidBoxOperator, compress: boolean = true): Promise<{ base64Data: string; mimeType: string }> {
...
export async function buildActionReturnValues(result: any, box: AndroidBoxOperator): Promise<{ content: Array<{ type: "text"; text: string } | { type: "image"; data: string; mimeType: string }> }> {
...
export async function getBoxCoordinates(box: AndroidBoxOperator, instruction: string): Promise<{ x: number; y: number }[]> {
  const screenshotUri = (await box.action.screenshot()).uri;
  const { base64Data, mimeType } = await getImageDataFromUri(screenshotUri, box);
  const coordinates = await getCUACoordinates(instruction, "data:" + mimeType + ";base64," + base64Data);
  ...
  const restoredCoordinates = coordinates.map(coordinate => restoreCoordinate(coordinate.x, coordinate.y, resizeRatio));
