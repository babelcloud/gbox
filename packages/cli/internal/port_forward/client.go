...
func (m *MultiplexClient) HandleAck(streamID uint32) {
    ...
    log.Printf("received ack for stream %d", streamID)
    stream.mu.Lock()
    if !stream.ready {
        stream.ready = true
...
func (m *MultiplexClient) HandleStream(stream *Stream) {
    ...
    for {
        ...
        default:
            ...
            if err != nil {
                log.Printf("localConn.Read error: %v", err)
                m.SendMessage(TypeClose, stream.id, nil)
                return
            }
...
func HandleLocalConnWithClient(localConn net.Conn, client *MultiplexClient, remotePort int) {
    defer func() {
        log.Printf("local tcp connection closed: %v", localConn.RemoteAddr())
        localConn.Close()
    }()
    ...
    log.Printf("stream %d opened for %v", streamID, localConn.RemoteAddr())
